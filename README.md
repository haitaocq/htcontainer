...
| `MULTICA_TOKEN` | 否 | Multica 认证 Token（可选增强；也支持 `MUL_TOKEN` 别名）。**仅当**设置了该变量或容器内已有有效认证配置时，才会登录并后台拉起 multica daemon，未配置不影响 dsh 正常运行。multica daemon 已内建 DSH runtime profile（`dsh --profile multica --probe`），无需手动安装插件 |
