...
- **特性**：
  ...
  - **multica 客户端（可选）**：内置 multica CLI，设置 `MULTICA_TOKEN`（或 `MUL_TOKEN` 别名）后自动鉴权并后台拉起 `multica daemon`（首次失败自动重试 30 次）；未配置时跳过，不影响 dsh 运行，支持 `MULTICA_SERVER_URL` / `MULTICA_APP_URL` 自定义服务地址；
