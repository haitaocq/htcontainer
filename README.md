...
| `agent_dsh` | `node:24-bookworm-slim` | [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)（dsh）+ multica | 内置 DeepSeek Harness Web UI / headless 任务的 Agent 环境 |
...
| `MULTICA_TOKEN` | 否 | Multica 认证 Token（可选增强；也支持 `MUL_TOKEN` 别名）。**仅当**设置了该变量或容器内已有有效认证配置时，才会登录并后台拉起 multica daemon，未配置不影响 dsh 正常运行 |
| `MULTICA_SERVER_URL` | 否 | 自定义 Multica 服务端地址（可选） |
| `MULTICA_APP_URL` | 否 | 自定义 Multica App 地址（可选） |
...
- **特性**：
  ...
  - **multica 客户端（可选）**：内置 multica CLI 与 **DSH multica runtime profile**（私有 `@multica-ai/dsh-runtime` 桥接包，构建期编译并装入 dsh `multica` profile，经 `dsh --profile multica --probe` 验证协议版本 1），multica daemon 即可注册 DeepSeek Harness 作为 agent runtime；设置 `MULTICA_TOKEN`（或 `MUL_TOKEN` 别名）后自动鉴权并后台拉起 `multica daemon`（首次失败自动重试 30 次）；未配置时跳过，不影响 dsh 运行，支持 `MULTICA_SERVER_URL` / `MULTICA_APP_URL` 自定义服务地址。启动 daemon 前会自动检测 runtime profile——即使旧具名卷未携带，也会从镜像出厂备份（`/usr/local/share/dsh/multica-profile`）复制补齐。注意：以 multica 运行时身份执行任务需在容器内提供模型凭证（`DEEPSEEK_API_KEY` 或 dsh 设置中的 provider）；
